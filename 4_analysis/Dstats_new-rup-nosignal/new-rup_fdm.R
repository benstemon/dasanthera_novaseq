...
setwd("~/project storage/project_dasanthera_novaseq/results/Dstats_new-rup-nosignal/")
...
save(filter_full, file = "~/project storage/project_dasanthera_novaseq/plot-making-compendium/new-rup-fdm_500bp_FILTERED_FULLSAMPLES.ggplot")



filter_nohybrid <- filterdf %>% filter(test == "nohybrid")
ggplot(data = filter_nohybrid, aes(x = midpoint, y = f_dM)) +
  geom_point(alpha = 0.01) +
  geom_smooth(linewidth = 0.5, method = "loess", se = F, span = 0.15) +
  facet_wrap(~chr, scales = "free_x", nrow = 2) +
  labs(x = "Position on scaffold (Mbp)",
       y = expression(italic(f[dM]))) +
  ggtitle("fdm -- CNR -- no hybrids") +
  theme_bw()


### THIS IS OLD CODE FROM THE PREVIOUS ATTEMPT -- KEPT AS EXAMPLE
