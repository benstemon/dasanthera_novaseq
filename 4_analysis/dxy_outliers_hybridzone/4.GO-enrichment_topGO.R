...
outlier_genes <- read.table("unique_mRNA_4z_dxy_hybridzone_outliers_filtered_deduplicate.bed",
                            header = F)
...
write.csv(tbl_bp, file="significant_GO_terms_BP.csv")
write.csv(tbl_mf, file="significant_GO_terms_MF.csv")
write.csv(tbl_cc, file="significant_GO_terms_CC.csv")

#2. genes of initial interest with those GO terms included
write.csv(data.frame(unique(genes_in_GO_bp)), file = "genes_of_interest_in_significant_GO_terms_BP.csv")
write.csv(data.frame(unique(genes_in_GO_mf)), file = "genes_of_interest_in_significant_GO_terms_MF.csv")
write.csv(data.frame(unique(genes_in_GO_cc)), file = "genes_of_interest_in_significant_GO_terms_CC.csv")
